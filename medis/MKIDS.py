...
class Camera():
    ...
    #         self.rebinned_cube = np.abs(
    #             np.sum(fields[:, -1], axis=2)) ** 2  # select detector plane and sum over objects
    ...
    # 
    #             if sp.verbose:
    #                 print(f"star flux: {ap.star_flux}, cube sum: {np.sum(
    #                     self.rebinned_cube)}, num fake events: {num_fake_events}")
    # 
            ...
            if self.product == 'photons':
                if num_chunks == 1:
                    ...
                    for ic in range(num_chunks):
                        ...
                        self.photons = self.get_photons(self.rebinned_cube[cspan[0]: cspan[1]],
                                                        chunk_step=abs_step + cspan[0])
                ...
                if num_chunks == 1:
                    ...
                    for ic in range(num_chunks):
                        ...
                        self.photons = self.get_photons(self.rebinned_cube[cspan[0]: cspan[1]],
                                                        chunk_step=abs_step + cspan[0])
                        self.rebinned_cube[cspan[0]: cspan[1]] = self.rebin_list(self.photons,
                                                                                 time_inds=[cspan[0], cspan[1]])
    ...
    def max_chunk(self, datacube, round_chunk=False):
        ...
        if round_chunk:
            ...
            while nice_cut <= max_chunk:
                nice_cut = round_nums[i % len(round_nums)] * 10 ** (
                            i // len(round_nums))  # [1,2,5,10,20,50,100,200,...]
                i += 1

            max_chunk = round_nums[(i - 2) % len(round_nums)] * 10 ** (
                        (i - 2) // len(round_nums))  # we want the nice value thats below max_chunk so go back 2
    ...
    def save_photontable(self, photonlist=[], index=('ultralight', 6), timesort=True, chunkshape=None, shuffle=True,
                         bitshuffle=False, ndx_shuffle=True, ndx_bitshuffle=False, populate_subsidiaries=True):
        """
        ...
        if populate_subsidiaries:
            ...
            if len(
                    out) > mkidcore.headers.METADATA_BLOCK_BYTES:  # this should match mkidcore.headers.ObsHeader.metadata
    ...
    def get_R_hyper(self, Rs, plot=False):
        ...
        waves = np.ones((np.shape(m)[1], np.shape(m)[0], ap.n_wvl_final + 5)) * np.linspace(ap.wvl_range[0],
                                                                                            ap.wvl_range[1],
                                                                                            ap.n_wvl_final + 5)
    ...
    def create_bad_pix_center(self, responsivities):
        ...
        for x in range(self.array_size[1]):
            ...
            dist = np.int_(dist(int(self.array_size[0] * mp.pix_yield))[
                               0])  # /float(mp.res_elements)*np.int_(self.array_size[0]) / mp.g_mean)
            # plt.plot(dist)
            # plt.show()
            dead_ind = []
    ...
    def get_bad_packets(self, type='dark'):
        ...
        if n_device_counts > 0:
            ...
            photons[0] = np.random.uniform(sp.startframe * sp.sample_time, sp.numframes * sp.sample_time,
                                           n_device_counts)
            photons[1] = phases
            photons[2:] = bad_pix
    ...
    def rebin_list(self, photons, time_inds=None):
        ...
        bins = [
            np.linspace(sp.sample_time * time_inds[0], sp.sample_time * time_inds[1], time_inds[1] - time_inds[0] + 1),
    ...
    def calc_arrival_diff(self, stem):
        print('Calculating the difference in arrival times')
        for x in range(len(stem)):
            for y in range(len(stem[0])):
                # print(x,y)
                if len(stem[x][y]) > 1:
                    # stem[x][y] = np.array(stem[x][y])
                    events = np.array(stem[x][y])
                    timesort = np.argsort(events[:, 0])
                    events = events[timesort]
                    events[:, 0] = events[:, 0] - np.roll(events[:, 0], 1, 0)
                    stem[x][y] = list(events)
    ...
    def rescale_cube(self, rebinned_cube, conserve=True):
        ...
        xnew = np.arange(-self.array_size[0] * self.platescale / 2, self.array_size[0] * self.platescale / 2,
                         self.platescale)
        ynew = np.arange(-self.array_size[1] * self.platescale / 2, self.array_size[1] * self.platescale / 2,
                         self.platescale)
